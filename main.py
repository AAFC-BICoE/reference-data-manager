...
def parse_input_args(argv):
    ...
    parser.add_argument('--update-ncbi-taxonomy', help="Update NCBI taxonomy database", dest="ncbi_taxonomy_update",
                        action = 'store_true', required=False)
    parser.add_argument('--restore-ncbi-taxonomy', help="Restore NCBI taxonomy database", dest="ncbi_taxonomy_restore",
                        required=False)
    # NCBI subsets (ITS, CO1, etc.). NCBI nrnt and NCBI taxonomy database are required to complete the whole update process. 
    parser.add_argument('--update-ncbi-subsets', help="Update NCBI subsets (ITS, CO1, etc.)", dest="ncbi_subsets_update",
                        action = 'store_true', required=False)
    parser.add_argument('--restore-ncbi-subsets', help="Restore NCBI's subsets (ITS, CO1, etc.)", dest="ncbi_subsets_restore",
                        required=False)
...
def execute_script(input_args):
    ...
    if parsed_args.ncbi_subsets_format:
        print("Format NCBI subsets: " )
        subsetData = NcbiSubsetData(config_file)
        success = subsetData.format()
        if success:
