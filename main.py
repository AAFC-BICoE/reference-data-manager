...
def parse_input_args(argv):
    ...
    parser = argparse.ArgumentParser(description=
            'Reference Data Manager (RDM) is used to download, update and \
            backup bioinformatics reference data. Note that only one action \
            (action examples: update, restore) is allowed each time to \
            execute the program.')
    ...
    parser.add_argument('--config-file', help='Path to the configuration file.\
            Can be ignored if there is a config.ini file in the brdm/ folder \
            (the same folder as file config.ini.sample).', dest='config_file',
            required=False)
    ...
    parser.add_argument('--restore-destination', help='Required by all the \
            database restore methods. Path to your restored database.', 
            dest='restore_destination')
    parser.add_argument('--restore-date', help='Required by all the database \
            restore methods. The version of the database to be restored; \
            format: yyyy-mm-dd.', dest='restore_source')
    # Download NCBI nrnt blast database
    parser.add_argument('--update-ncbi-blast', help='Download NCBI nr/nt \
            blast database.', dest='ncbi_blast_update', action='store_true', 
            required=False)
    # Unzip NCBI nrnt blast database
    parser.add_argument('--unzip-ncbi-blast', help='Unzip NCBI nr/nt blast \
            database.', dest='ncbi_blast_unzip', action='store_true', 
            required=False)
    # Download NCBI taxonomy database
    parser.add_argument('--update-ncbi-taxonomy', help='Update NCBI taxonomy \
            database.', dest="ncbi_taxonomy_update", action = 'store_true', 
            required=False)
    ...
    parser.add_argument('--restore-ncbi-taxonomy', help='Restore NCBI taxonomy\
            database. --restore-destination and --restore-source are required',
            dest="ncbi_taxonomy_restore", action='store_true',required=False)
    # NCBI subsets database(ITS, CO1, etc.). NCBI nrnt and NCBI taxonomy 
    # database are required to complete the whole update process. 
    parser.add_argument('--update-ncbi-subsets', help='Update NCBI subsets \
            (ITS, CO1, etc.)', dest='ncbi_subsets_update',action='store_true',
            required=False)
    ...
    parser.add_argument('--update-ncbi-wholegenomes', help='Update NCBI whole \
            genomes', dest='ncbi_whole_genome_update', action='store_true', 
            required=False)
    parser.add_argument('--restore-ncbi-wholegenomes', help='Restore NCBI \
            whole genomes. --restore-destination and --restore-source \
            are required', dest='ncbi_whole_genome_restore',
            action = 'store_true', required=False)
    # Green gene
    parser.add_argument('--update-greengene', help='Update GreenGene data', 
            dest='greengene_update', action='store_true', required=False)
    parser.add_argument('--format-greengene', help='Format GreenGene data', 
            dest='greengene_format',action='store_true', required=False)
    # Unite data
    parser.add_argument('--update-unitedata', help='Update unite data', 
            dest='unitedata_update', action='store_true', required=False)
